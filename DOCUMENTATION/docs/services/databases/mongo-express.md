...
| `ME_PORT` | `8081` | Host-side port the UI is published on (`host:container`, container port is fixed at `8081` upstream). |
...
- **Port already in use on your host.** Another local service is bound to `8081`. Change `ME_PORT` in `.env` and `./laradock restart mongo-express`.
